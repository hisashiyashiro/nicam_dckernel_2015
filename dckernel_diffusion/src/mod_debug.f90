...
! The second argument should correspond to ADM_prc_me, i.e. 1, 2, ..
  subroutine snapshot_seq_bin_open (prefix, my_id)
    ...
    if(trim(prefix).eq.string_null) then
        my_snapshot = 0
        my_id_saved = my_id
        write(0,'(a)') "<snapshot_seq_bin_open> testing without snapshot"
        return
    endif
  ...
  subroutine ADM_ascii_write
    ...
    write(fname,'(a,i3.3)') "ADM_ascii_write.", my_id_saved
    ...
    write(my_ascii,'(a,10i8)') "ADM_lall, ADM_lall_pl:", ADM_lall, ADM_lall_pl
    write(my_ascii,'(a,10i8)') "ADM_gall, ADM_gall_pl:", ADM_gall, ADM_gall_pl
    write(my_ascii,'(a,10i8)') "ADM_kall:", ADM_kall
    write(my_ascii,'(a,10i8)') "ADM_gall_1d, ADM_gslf_pl:", ADM_gall_1d, ADM_gslf_pl
    ...
    write(my_ascii,'(a,10i8)') "l, nstart, nend=", l, nstart, nend
    enddo

    do l = 1, ADM_lall
    write(my_ascii,'(a,i3,2x, L2)') "l, ADM_have_sgp(l):", l, ADM_have_sgp(l)
    enddo
    close(my_ascii)
    return
